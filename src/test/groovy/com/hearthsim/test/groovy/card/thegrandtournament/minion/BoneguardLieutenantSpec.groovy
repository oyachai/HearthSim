...
import com.hearthsim.card.thegrandtournament.minion.common.BoneguardLieutenant
 ...
 */
class BoneguardLieutenantSpec extends CardSpec {
    ...
    def setup() {
        startingBoard = new BoardModelBuilder().make(new Priest(), new TestHero(), {
            currentPlayer {
                heroHealth(10)
                field([[minion:BoneguardLieutenant]])
                mana(10)
            }
            waitingPlayer {
                field([[minion:BoneguardLieutenant]])
