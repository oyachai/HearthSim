...
class TwilightWhelpSpec extends CardSpec {
    ...
    def "no buffs when not holding a dragon"() {
        ...
        assertBoardDelta(copiedBoard, ret.data_) {
            currentPlayer {
                playMinion(TwilightWhelp, CharacterIndex.HERO)
                mana(9)
                numCardsUsed(1)
            }
        }
    ...
    def "triggers buffs when holding a dragon"() {
        ...
        assertBoardDelta(copiedBoard, ret.data_) {
            currentPlayer {
                ...
                mana(9)
                numCardsUsed(1)
                updateMinion(CharacterIndex.MINION_1, [deltaHealth: 2, deltaMaxHealth: 2])
            }
        }
    }
