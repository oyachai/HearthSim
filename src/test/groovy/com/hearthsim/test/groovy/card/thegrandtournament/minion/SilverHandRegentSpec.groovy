...
import com.hearthsim.card.thegrandtournament.minion.common.SilverHandRegent
 ...
 */
class SilverHandRegentSpec extends CardSpec {
    HearthTreeNode root
    ...
    def setup() {
        startingBoard = new BoardModelBuilder().make(new Priest(), new TestHero(), {
            currentPlayer {
                heroHealth(10)
                field([[minion:SilverHandRegent], [minion: WarGolem]])
                mana(10)
            }
            waitingPlayer {
                field([[minion:SilverHandRegent], [minion: WarGolem]])
    ...
    def "using Hero ability buffs the minion"() {
        ...
        assertBoardDelta(copiedBoard, ret.data_) {
            currentPlayer {
                ...
                addMinionToField(SilverHandRecruit, CharacterIndex.MINION_1)
            }
        }
    }
}
